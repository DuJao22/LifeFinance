    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-2 sm:p-4 bg-slate-950/85 backdrop-blur-md font-sans overflow-y-auto">
        <motion.div
          ...
          className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 max-w-lg w-full shadow-2xl relative text-slate-900 dark:text-slate-100 max-h-[92vh] flex flex-col my-auto"
        >
          {/* Header */}
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800 shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold flex items-center justify-center text-lg overflow-hidden border border-slate-200 dark:border-slate-700 shrink-0">
                {record.photo ? (
              ...
              <div>
                <h3 className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white">{record.name}</h3>
                <p className="text-[11px] text-slate-500 dark:text-slate-400">{record.description}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {/* Key Stats Box */}
          <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 mb-4 space-y-2 text-xs">
            ...
            <div className="flex justify-between font-bold text-sm pt-2 border-t border-slate-200 dark:border-slate-700">
              <span className="text-slate-800 dark:text-slate-200">Saldo Restante:</span>
              <span className={remaining > 0 ? (isDevo ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400') : 'text-slate-400'}>
          ...
          <div className="flex gap-2 mb-6">
            ...
            {record.phone && (
              <a
                ...
                className="py-2.5 px-4 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5"
              >
          ...
          <div>
            ...
            {personHistory.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-4 bg-slate-100 dark:bg-slate-800/40 rounded-xl">
              ...
              <div className="space-y-2">
                {personHistory.map((h) => (
                  <div
                    key={h.id}
                    className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/40 text-xs flex items-center justify-between"
                  >
                    <div>
                      <p className="font-semibold text-slate-800 dark:text-slate-200">{h.description}</p>
