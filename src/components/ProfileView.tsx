...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-24 font-sans max-w-4xl mx-auto">
      {/* Profile Header */}
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-xl backdrop-blur-md flex flex-col sm:flex-row items-center gap-6 text-slate-900 dark:text-slate-100">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <div className="mt-2 flex flex-wrap justify-center sm:justify-start gap-2 text-[10px] text-slate-500 dark:text-slate-400">
            ...
            <span className="px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30">
              Criptografia Ativa
            </span>
          </div>
        </div>
      ...
      {/* Profile Edit Form */}
      <form onSubmit={handleSaveProfile} className="p-6 rounded-3xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-xl backdrop-blur-md space-y-4">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-3">Informações Pessoais</h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Nome Completo</label>
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900/90 border border-emerald-500/30 shadow-xl backdrop-blur-md space-y-3">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Proteção de ponta a ponta com criptografia AES-256-GCM & PBKDF2</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono pt-1">
          <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 block">ALGORITMO</span>
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">{encMeta.engine}</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 block">CHAVE MASTER</span>
            <span className="text-blue-600 dark:text-blue-400 font-bold">{encMeta.algorithm}</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 block">TAMANHO DA CHAVE</span>
            <span className="text-amber-600 dark:text-amber-300 font-bold">{encMeta.bitLength} Bits</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 block">STATUS</span>
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">🔒 Criptografado</span>
          </div>
        </div>
      </div>

      {/* Reports, Backup, Restore & Zerar Dados Section */}
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-xl backdrop-blur-md space-y-4">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center justify-between">
          <span>Relatórios, Backup & Gestão de Dados</span>
          <span className="text-[11px] font-normal text-slate-500 dark:text-slate-400">
            Total de Lançamentos: <strong className="text-slate-900 dark:text-white">{records.length}</strong>
          </span>
        </h3>
        ...
        {showPasswordModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative text-slate-900 dark:text-slate-100"
            >
              ...
              <form onSubmit={handleChangePassword} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-xs focus:outline-none focus:border-blue-500"
                  />
                </div>
                <div>
        ...
        {showResetModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className="bg-white dark:bg-slate-900 border border-amber-500/40 rounded-3xl p-6 max-w-md w-full shadow-2xl relative text-slate-900 dark:text-slate-100 text-center space-y-4"
            >
              <div className="w-12 h-12 rounded-2xl bg-amber-500/20 text-amber-500 dark:text-amber-400 flex items-center justify-center mx-auto">
                <AlertTriangle className="w-6 h-6" />
              </div>

              <div>
                <h3 className="text-base font-bold text-amber-600 dark:text-amber-400">Zerar Todos os Dados Financeiros?</h3>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 bg-slate-100 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="text-left space-y-1.5">
                <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300">
                  Digite <span className="text-amber-600 dark:text-amber-400 font-extrabold">ZERAR</span> para confirmar:
                </label>
                <input
                  type="text"
                  placeholder="ZERAR"
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs font-mono font-bold text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 uppercase"
                />
              </div>
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className="bg-white dark:bg-slate-900 border border-rose-300 dark:border-rose-900/50 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative text-slate-900 dark:text-slate-100 text-center"
            >
              <Trash2 className="w-12 h-12 text-rose-500 mx-auto mb-3" />
              <h3 className="text-base font-bold text-rose-600 dark:text-rose-400">Excluir Conta Permanentemente?</h3>
