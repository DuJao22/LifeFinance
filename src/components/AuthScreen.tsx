...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess, showToast }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex items-center justify-center p-4 relative overflow-hidden font-sans transition-colors duration-200">
      ...
      <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-emerald-600/10 dark:bg-emerald-600/20 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <motion.div
            ...
            className="inline-flex w-16 h-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden p-0.5 mb-3"
          >
            <img
              src={LOGO_DATA_URL}
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">LifeFinance</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Gestão inteligente de dívidas e empréstimos pessoais</p>
        </div>

        {/* Auth Form Container */}
        <motion.div
          layout
          className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl"
        >
          {/* Mode Switch Tabs */}
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
                    />
                  </div>
                </motion.div>
              )}
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                <label className="flex items-center gap-2 cursor-pointer text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100">
                  <input
                    ...
                    className="w-4 h-4 rounded bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500"
                  />
                  Lembrar de mim
                </label>
                <button
          ...
          {/* Security badge */}
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-center gap-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        {showForgotModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl relative text-slate-900 dark:text-slate-100"
            >
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-2">Recuperar Senha</h3>
              ...
              {forgotSent ? (
                <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-xl text-center space-y-2">
                  ...
                  <button
                    ...
                    className="mt-2 w-full py-2 bg-slate-100 dark:bg-slate-800 text-xs font-semibold rounded-lg text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700"
                  >
                    Fechar
                  </button>
                </div>
                ...
                <form onSubmit={handleForgotSubmit} className="space-y-4">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm focus:outline-none focus:border-blue-500"
                  />
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setShowForgotModal(false)}
                      className="flex-1 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700"
                    >
                      Cancelar
                    </button>
                    <button
