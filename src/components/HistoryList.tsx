    ...
    <div className="space-y-6 pb-24 font-sans">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-slate-100 text-xs focus:outline-none focus:border-blue-500"
            />
          </div>

          <div className="flex gap-1.5 bg-slate-800/80 p-1 rounded-xl shrink-0">
            <button
              onClick={() => setTypeFilter('TODOS')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                typeFilter === 'TODOS' ? 'bg-blue-600 text-white' : 'text-slate-400'
              }`}
            >
              Todos
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                typeFilter === 'DEVO' ? 'bg-rose-600 text-white' : 'text-slate-400'
              }`}
            >
              Pagamentos
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                typeFilter === 'RECEBER' ? 'bg-emerald-600 text-white' : 'text-slate-400'
              }`}
            >
              Recebimentos
            </button>
      ...
      {filteredHistory.length === 0 ? (
        <div className="text-center py-12 bg-slate-900/50 rounded-3xl border border-slate-800/60 p-6">
          ...
          <p className="text-sm font-bold text-slate-300">Nenhum registro no histórico</p>
          ...
          {filteredHistory.map((item) => {
            ...
            return (
              <motion.div
                ...
                className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800/80 shadow-lg flex items-center justify-between gap-4 text-xs"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-slate-100">{item.personName}</span>
